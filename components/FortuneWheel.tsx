...
const BONUSES: Bonus[] = [
  ...
  },
  {
    label: "20 FS",
    value: "20 фриспинов",
    description: "Фриспины на слот Pulz Originals",
  },
  {
    label: "+25%",
    value: "25%",
    description: "Повышенный кэшбек",
  },
  {
    label: "50 FS",
    value: "50 фриспинов",
    description: "Большой пак фриспинов",
  },
  {
    label: "MYSTERY",
    value: "Секретный бонус",
    description: "Персональное предложение после регистрации",
  },
];
...
function CircleButton({ label, onClick, important, children }: CircleButtonProps) {
  return (
    <button
      onClick={onClick}
      className={[
...
function MoneyBagIcon() {
  return (
    <svg viewBox="0 0 40 40" className="h-6 w-6">
      ...
      <path
        d="M15 8c0-1.7 1.3-3 3-3h4c1.7 0 3 1.3 3 3l-1.5 3h-7L15 8z"
        fill="#1f2937"
      />
      <path
  ...
  return (
    <svg viewBox="0 0 40 40" className="h-6 w-6">
      ...
      <rect
        ...
        strokeWidth="1"
      />
      <rect x="11" y="15" width="18" height="8" rx="1.5" fill="#111827" />
    ...
    <>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-0 z-30 flex justify-center pb-4">
        <div className="pointer-events-auto mx-auto flex max-w-lg flex-1 items-center justify-between gap-4 rounded-full border border-red-900/40 bg-black/80 px-5 py-3 shadow-[0_-8px_35px_rgba(0,0,0,0.9)]">
          <CircleButton label="Касса" onClick={openCashier}>
            <MoneyBagIcon />
          </CircleButton>

          <CircleButton label="Вход" onClick={openLogin}>
            <UserIcon />
          </CircleButton>
          ...
          <CircleButton label="Игры" onClick={openGames}>
            <SlotIcon />
          </CircleButton>

          <CircleButton label="Меню" onClick={openMenu}>
            <MenuIcon />
          </CircleButton>
        </div>
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          {/* Лёгкое затемнение вокруг колеса */}
          ...
          <div className="relative mx-4 w-full max-w-md rounded-3xl border border-red-900/60 bg-gradient-to-b from-[#05040a] via-[#150410] to-[#020106] p-5 shadow-[0_0_55px_rgba(248,113,113,0.7)]">
            <div className="mb-4 flex items-start justify-between">
              ...
              <button
                ...
                className="rounded-full border border-slate-700/70 bg-black/60 px-3 py-1 text-xs text-slate-300 hover:bg-slate-800"
              >
                Закрыть
              </button>
            </div>
            ...
            <div className="flex flex-col items-center gap-4">
              <div className="relative h-56 w-56">
                ...
                />
                {/* Центральный диск */}
                <div className="absolute inset-12 rounded-full bg-gradient-to-b from-[#ffe8a3] via-[#ffb74d] to-[#f97316] border border-amber-500/60 flex items-center justify-center">
                  <button
                    onClick={handleSpin}
                    disabled={isSpinning}
                ...
                {bonus ? (
                  <>
                    ...
                    <div className="mt-3 text-[11px] text-slate-500">
                      Забрать бонус можно после регистрации. Мы сохраним
                      результат этого спина.
                    </div>
                  </>
                ) : (
                  <div className="text-[13px] text-slate-400">
                    Нажми <span className="font-semibold text-slate-100">SPIN</span>, чтобы
                    получить демо-бонус Pulz Wheel. Реальные бонусы будут
                    доступны после запуска платформы.
                  </div>
                )}
              </div>
            </div>
